...
from photo_ocr.detection.craft.postprocessing.postprocessing import postprocess
from photo_ocr.detection.craft.old import adjustResultCoordinates
...
class TextDetector:
    ...
    def detect(self,
        ...
        resized, ratio = prepare_image(image, canvas_size, mag_ratio)
        ...
        # Post-processing
        boxes, polys = postprocess(score_text, score_link, text_threshold, link_threshold, low_text)
        refactoraid.add("boxes", boxes)
        refactoraid.add("polys", polys)

        # coordinate adjustment
        boxes = adjustResultCoordinates(boxes, ratio, ratio)
        polys = adjustResultCoordinates(polys, ratio, ratio)
        for k in range(len(polys)):
            if polys[k] is None: polys[k] = boxes[k]

        return list(zip(boxes, polys))
